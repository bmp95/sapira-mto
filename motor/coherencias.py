...
def comprobar(linea: LineaSalida, interruptores: dict[str, bool]) -> list[Motivo]:
    ...
    if interruptores.get("calidad_solo_tuerca") and calidad in {"8", "10"} and nombre and nombre != "TUERCA":
        motivos.append(Motivo(
            codigo="CALIDAD_SOLO_TUERCA", atributo="calidad",
            texto=f"La calidad {calidad} s{chr(0xf3)}lo aplica a tuercas (§5) y esto es {nombre}."))

    if (interruptores.get("calidad_solo_arandela") and calidad.endswith("HV")
            and nombre and nombre != "ARANDELA"):
        motivos.append(Motivo(
            codigo="CALIDAD_SOLO_ARANDELA", atributo="calidad",
    ...
    if interruptores.get("inox_acabado") and calidad in _INOX and acabado in _ZINC:
        ...
        motivos.append(Motivo(
            codigo="INOX_CON_ACABADO_ZINC", atributo="acabado",
            texto=f"{calidad} es inox {_austenitico}, as{chr(0xed)} que un acabado "
                  f"{acabado.lower()} es incoherente."))
    ...
    if interruptores.get("sistema_medida") and norma and medida:
        ...
        if imperial_norma and metrica_medida:
            motivos.append(Motivo(
                codigo="SISTEMA_MEDIDA_INCOHERENTE", atributo="medida",
    ...
    if interruptores.get("grado_astm_nombre") and nombre:
        if calidad in {"GR 2H", "2H"} and nombre != "TUERCA":
            motivos.append(Motivo(
                codigo="GRADO_ASTM_INCOHERENTE", atributo="calidad",
    ...
    if interruptores.get("material_vs_calidad") and material and calidad:
        ...
        if mat_calidad:
            ...
            if material != mat_esperado:
                motivos.append(Motivo(
                    codigo="MATERIAL_CONTRADICE_CALIDAD", atributo="material",
                    texto=f"La calidad {calidad} implica material {mat_esperado}, "
                          f"pero se indica {material}."))
    ...
    if interruptores.get("material_vs_norma") and material and norma:
        ...
        if mat_norma:
            ...
            if material != mat_esperado:
                motivos.append(Motivo(
                    codigo="MATERIAL_CONTRADICE_NORMA", atributo="material",
    ...
    if interruptores.get("nombre_vs_norma") and nombre and norma:
        ...
        if nom_norma:
            ...
            if nombre != nom_esperado:
                ...
                if es_equivalencia:
                    motivos.append(Motivo(
                        codigo="NOMBRE_Y_NORMA_EQUIVALENTES",
                        texto=f"La norma {norma} sugiere {nom_esperado}, pero se indica "
                              f"{nombre} — equivalentes en el oficio."))
                else:
                    motivos.append(Motivo(
                        codigo="NOMBRE_CONTRADICE_NORMA", atributo="nombre",
    ...
    if interruptores.get("material_vs_acabado"):
        if material == "INOX" and acabado in _ZINC:
            ...
            motivos.append(Motivo(
                codigo="INOX_CON_ACABADO_ZINC", atributo="acabado",
                texto=f"{material} es inox {_austenitico}, as" + chr(0xed) +
                f" que un acabado {acabado.lower()} es incoherente."))
        elif material in _NO_ZINC and acabado in _ZINC:
            motivos.append(Motivo(
                codigo="MATERIAL_NO_ADMITE_ZINC", atributo="acabado",
    ...
    if interruptores.get("longitud_tuerca_arandela") and longitud and nombre in {"TUERCA", "ARANDELA"}:
        motivos.append(Motivo(
            codigo="LONGITUD_INESPERADA", atributo="longitud",
        ...
        try:
            if medida.startswith("M"):
                ...
                if longitud_num < medida_num:
                    motivos.append(Motivo(
                        codigo="LONGITUD_IMPOSIBLE", atributo="longitud",
